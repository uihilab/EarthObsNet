...
import torchvision
from dataPrj_SA_data import CarvanaDataset
...
def validation_loop(loader, model, loss_fn):
    ...
    with torch.no_grad():
        for x_outside, x_current, y in loader:
            x_outside = x_outside.cuda()
            x_current = x_current.cuda()
            outputs = model(x_outside, x_current).cuda()
            target = y.cuda()
...
def save_predictions_as_imgs(
    ...
    torch.cuda.empty_cache()
    with torch.no_grad(): 
        for idx, (x_outside, x_current, y) in enumerate(loader):
            output = model(x_outside.to(device), x_current.to(device))
            target = y.to(device)
            
            for itemidx in range(target.shape[0]):

                # compute MAE, NSE, and EML (referred to as SD in the following papers: https://doi.org/10.1016/j.isprsjprs.2023.10.002
        ...
        with open(f"dataPrj_files/mergeInput/NSE_{fileName}.txt", "w") as NSE_file:
            json.dump(NSE, NSE_file)   
        with open(f"dataPrj_files/mergeInput/MAE_{fileName}.txt", "w") as MAE_file:
            json.dump(MAE, MAE_file)
        with open(f"dataPrj_files/mergeInput/EML_{fileName}.txt", "w") as EML_file:
            json.dump(EML, EML_file)
        with open(f"dataPrj_files/mergeInput/AAI3_{fileName}.txt", "w") as AAI3_file:
            json.dump(AAI3.tolist(), AAI3_file)
